...
2. `Cursor/Claude Code가 사내 규칙을 자꾸 어겨서 만든 도구 (한국 도메인 룰 20개 포함)`
3. `금감원 AI 가이드라인 + PCI DSS를 룰셋으로 정형화 — aicqtools v1.0-alpha 공개`
...
요즘 Cursor / Claude Code로 코드 짜는 분들 많으실 텐데, 제 경험으로는 **사내 규칙을 자꾸 어기는 게 진짜 골치**더라구요. 예를 들어:
...
3. **MCP 네이티브** — Claude Code / Cursor MCP 서버로 등록하면 **AI가 코드를 만들기 전 차단** (사후 검증이 아닌 사전). 글로벌 가드레일 도구들은 PR 시점에 코멘트 다는 사후 패턴.

4. **하이브리드 룰 DSL** — 간단한 패턴은 YAML 5줄, 복잡한 룰은 TS 함수 (`defineRule({ visitors: { call_expression: ... } })`).

5. **출처 추적기 모듈** — Git pre-commit으로 Claude Code 세션 자동 캡처 → CycloneDX AI-BOM + EU AI Act Article 50 한·영 HTML/PDF 리포트. **EU AI Act 시행일 2026-08-02** 직격.

## 라이선스 / 가격

- 엔진은 **MIT OSS**, 영구 무료
- 클라우드 대시보드(v1.5 예정)는 **₩29,000/repo/월** (Semgrep Team $35 × 인원 모델 대비 repo당 정액)
- 1인 개발자는 무료, 팀 결제
...
5. **라이선스 / 가격** 모델 의견 — repo당 정액이 per-seat 대비 어떤가요?
